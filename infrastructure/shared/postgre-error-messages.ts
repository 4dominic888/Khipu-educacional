...
export function postgreDefaultErrorMessage(error?: PostgrestError, defaultMessage?: string): string {
  if (!error) return 'Ocurrió un error inesperado. Intenta nuevamente.';

  const startsWith = (prefix: string) => error.code.startsWith(prefix);
